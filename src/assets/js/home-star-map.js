(function () {
  ...
  // --- Intro transition layout preview -----------------------------------
  // The entry sequence (site-visuals.js) dissolves its logo particles into
  // this star map. To let those particles land exactly where each star will
  // be rendered, the 2D contribution layout is computed here — as soon as
  // the visual settings resolve, long before GCK_HOME_INTRO_READY fires —
  // and shared through window.GCK_HOME_STAR_LAYOUT. The layout is
  // deterministic: createContributionMap seeds its randomness with the
  // graph revision, so mirroring the same construction and random
  // consumption order yields identical positions. WebGL modes arrange
  // stars differently, but the 2D layout is close enough to serve as the
  // transition target for every render mode.
  function computeStarLayoutPreview(resolved) {
    const previewSettings = normalizeSettings({
      ...defaults,
      ...(resolved || {})
    });
    if (previewSettings.home_background_style !== "contribution_star_map") {
      return null;
    }
    if (resolved && resolved.contribution_graph) {
      cacheContributionGraph(resolved.contribution_graph);
    }
    const sourceGraph = graphWithCachedContributions(
      graph,
      readContributionGraph()
    );
    const scope = previewSettings.home_star_scope;
    const bounds =
      scope === "full"
    ...
    const random = seededRandom(
      hashSeed(`${graph.revision}:contribution-stars`)
    );
    const stars = sourceGraph.stars.map((source, index) => {
      if (source.kind === "document") {
        // The renderer spends two random draws on drift velocities.
        random();
        random();
      }
      const baseBrightness = formulaEngine.calculateBrightness(
        source,
        previewSettings.home_star_brightness_rules,
        previewSettings.home_star_brightness_min,
        previewSettings.home_star_brightness_initial,
        previewSettings.home_star_brightness_max
      );
      return {
        id: source.id,
        kind: source.kind,
        index,
        color: previewSettings.home_star_color_random_enabled
    ...
    // positionStars(true) from createContributionMap: contributors sit on a
    // golden-angle spiral, documents take seeded random positions.
    stars
      .filter((star) => star.kind === "contributor")
      .forEach((star, index) => {
        const angle =
    ...
    // The intro stage sits in flow above the page; when the sequence
    // finishes it is removed (lock-scroll mode) or scrolled past by exactly
    // its own height, so the canvas ends up one stage-height higher in the
    // viewport than it is right now.
    let offsetLeft = 0;
    let offsetTop = 0;
    if (scope !== "full") {
      const stage = document.querySelector("[data-entry-sequence]");
      const stageHeight = stage ? stage.getBoundingClientRect().height : 0;
      const rectangle = canvas.getBoundingClientRect();
      offsetLeft = rectangle.left + window.scrollX;
      offsetTop = rectangle.top + window.scrollY - stageHeight;
    }
    return {
      scope,
      width,
      ...
      stars: stars.map((star) => {
        const tier = formulaEngine.brightnessTier(
          star.baseBrightness,
          previewSettings.home_star_brightness_tiers,
          previewSettings.home_star_brightness_min,
          previewSettings.home_star_brightness_max
        );
        const profile = tierProfile(tier);
        const presentation = illumination.brightnessPresentation(
          ...
          previewSettings.home_star_brightness_max,
          {
            radiusBoost: previewSettings.home_star_selected_radius_boost,
            alphaBoost: previewSettings.home_star_selected_alpha_boost,
            haloAlphaBoost:
              previewSettings.home_star_selected_halo_alpha_boost,
            glowScale: previewSettings.home_star_selected_glow_scale
          }
        );
        const glow = mixRgb(star.color, profile.tintHex, profile.tintMix);
        return {
          id: star.id,
          kind: star.kind,
          x: star.x + offsetLeft,
          ...
          tier: tier ? tier.id : "",
          color: `rgba(${glow.r},${glow.g},${glow.b},1)`,
          radius:
            presentation.radius *
            (1 + profile.radiusBoost * presentation.luminous),
          haloRadius: presentation.haloRadius * profile.haloScale,
          alpha: presentation.alpha,
          haloAlpha: presentation.haloAlpha * profile.haloAlphaScale
        };
      })
    };
  }

  // Exposed as a promise so the entry sequence can await it with a fallback
  // timeout. Resolves to null whenever the contribution star map is not the
  // active background or the layout cannot be computed.
  window.GCK_HOME_STAR_LAYOUT = Promise.resolve(
    window.GCK_VISUAL_SETTINGS || {}
  )
    .then((resolved) => {
      try {
        return computeStarLayoutPreview(resolved || {});
      } catch {
        return null;
      }
    })
