...
function loadCodeProjects(sourceRoot) {
  const roots = [
    path.join(sourceRoot, "program", "code"),
    path.join(sourceRoot, "code")
  ];
  const manifests = roots.flatMap((root) => discoverManifests(root));
  const projects = manifests.map((manifest) => loadProject(manifest, sourceRoot));
  const ids = new Set();
