...
function parseHistory(sourceRoot, range) {
  ...
  for (const block of output.split("\x1e")) {
    ...
    for (const line of lines) {
      ...
      if (!track) continue;
      const currentPath = currentPathFromNumstat(match[3]);
      const stats = lineStatsFromNumstat(match[1], match[2]);
      paths.add(currentPath);
      if (!pathStats[currentPath]) pathStats[currentPath] = emptyLineStats();
      addLineStats(pathStats[currentPath], stats);
      addLineStats(
        tracks[track],
        stats
      );
...
function aggregateStatistics(
  ...
  const parsedReferenceTime = new Date(referenceDate).getTime();
  const referenceTime = Number.isFinite(parsedReferenceTime)
    ? parsedReferenceTime
    : Date.now();
  const sevenDayBoundary = referenceTime - 7 * 24 * 60 * 60 * 1000;
  const thirtyDayBoundary = referenceTime - 30 * 24 * 60 * 60 * 1000;
  const events = [];
  ...
  function buildScope(key, title, trackKeys) {
    ...
    for (const commit of identifiedCommits) {
      ...
      if (
        ...
        timestamp < thirtyDayBoundary ||
        timestamp > referenceTime
      ) {
        continue;
      }
      const recentStats = trackKeys.reduce((total, trackKey) => {
        return addLineStats(
          total,
          commit.tracks[trackKey] || emptyLineStats()
        );
      }, emptyLineStats());
      const modificationCount =
        recentStats.added + recentStats.modified + recentStats.deleted;
      if (!modificationCount) continue;
      const contributor = contributors.get(commit.contributorId);
      if (!contributor) continue;
      contributor.activity30Count += 1;
      contributor.modification30Count += modificationCount;
      if (timestamp >= sevenDayBoundary) {
        contributor.activity7Count += 1;
        contributor.modification7Count += modificationCount;
      }
    }
  ...
  for (const commit of identifiedCommits) {
    ...
    for (const [documentPath, lineStats] of Object.entries(
      commit.pathStats || {}
    )) {
      if (!documentMetrics.has(documentPath)) {
        const metric = {
          path: documentPath,
          activity7Count: 0,
          activity30Count: 0,
        ...
        Object.defineProperty(metric, "_commitWindows", {
          value: {
            seven: new Set(),
            thirty: new Set()
          },
        ...
        documentMetrics.set(documentPath, metric);
      }
      const metric = documentMetrics.get(documentPath);
      const modificationCount =
        Number(lineStats.added || 0) +
        Number(lineStats.modified || 0) +
        Number(lineStats.deleted || 0);
      metric._commitWindows.thirty.add(commit.sha);
      metric.modification30Count += modificationCount;
      if (timestamp >= sevenDayBoundary) {
        metric._commitWindows.seven.add(commit.sha);
        metric.modification7Count += modificationCount;
      }
    }
  }
  for (const metric of documentMetrics.values()) {
    metric.activity7Count = metric._commitWindows.seven.size;
    metric.activity30Count = metric._commitWindows.thirty.size;
  }

  return {
    scopes,
    recentEvents,
    documentMetrics: Array.from(documentMetrics.values()).sort(
      (left, right) => {
...
function getContentStatistics(sourceRoot, options = {}) {
  const cachePath =
    ...
    path.resolve(process.cwd(), ".cache/content-statistics-v4.json");
