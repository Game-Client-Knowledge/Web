...
const DEFAULT_BRIGHTNESS_RULES = [
  ...
  },
  {
    ...
    formula:
      ...
      "0.42 * min(1, log(1 + reading_seconds) / log(1000001)) + " +
      "0.28 * min(1, log(1 + view_count) / log(10001)) + " +
      "0.20 * min(1, sqrt(average_reading_seconds / 900)))"
  },
  {
...
const GRAPH_SCALED_DEFAULT_FORMULAS = new Map([
  [
    "contributor-total",
...
const PRODUCTION_DEFAULT_FORMULAS = new Map([
  [
    "contributor-total",
    "current_brightness + brightness_span * 0.65 * " +
  ...
  ["document-reference", PREVIOUS_DEFAULT_FORMULAS.get("document-reference")],
  [
    "document-contributors",
    PREVIOUS_DEFAULT_FORMULAS.get("document-contributors")
  ],
  ["document-recent", PREVIOUS_DEFAULT_FORMULAS.get("document-recent")]
]);
const STANDARD_PREVIOUS_NAMES = new Map([
  ["contributor-total", "静星累计贡献"],
  ["contributor-recent", "静星近期修改"],
  ["document-reference", "动星引用关系"],
  ["document-strong", "动星强联系"],
  ["document-contributors", "动星贡献者"],
  ["document-recent", "动星近期修改"]
]);
const PRODUCTION_DEFAULT_NAMES = new Map([
  ...STANDARD_PREVIOUS_NAMES,
  ["contributor-recent", "静星30日修改"],
  ["rule-3f239463", "静星7日修改"]
]);
const STANDARD_PREVIOUS_ORDERS = [
  [
    "contributor-total",
    "contributor-recent",
    "document-reference",
...
const PREVIOUS_DEFAULT_CONFIGURATIONS = [
  {
    formulas: PREVIOUS_DEFAULT_FORMULAS,
    names: STANDARD_PREVIOUS_NAMES,
    orders: STANDARD_PREVIOUS_ORDERS
  },
  {
    formulas: REBALANCED_DEFAULT_FORMULAS,
    names: STANDARD_PREVIOUS_NAMES,
    orders: STANDARD_PREVIOUS_ORDERS
  },
  {
    formulas: GRAPH_SCALED_DEFAULT_FORMULAS,
    names: STANDARD_PREVIOUS_NAMES,
    orders: STANDARD_PREVIOUS_ORDERS
  },
  {
    formulas: PRODUCTION_DEFAULT_FORMULAS,
    names: PRODUCTION_DEFAULT_NAMES,
    orders: [[
      "contributor-total",
      "contributor-recent",
      "rule-3f239463",
...
const PREVIOUS_CONTRIBUTOR_TOTAL_FORMULAS = new Set(
  PREVIOUS_DEFAULT_CONFIGURATIONS.map((configuration) => {
    return configuration.formulas.get("contributor-total");
  })
);
const VARIABLE_DEFINITIONS = Object.freeze([
  ...
  },
  {
    id: "view_count",
    label: "累计阅览次数",
    description: "动星对应文件的累计阅览次数；静星汇总其贡献文件获得的阅览。"
  },
  {
    id: "reading_seconds",
    label: "累计阅读时长",
    description: "动星对应文件的累计阅读秒数；静星汇总其贡献文件获得的时长。"
  },
  {
    id: "average_reading_seconds",
    label: "平均阅读时长",
    description: "累计阅读秒数除以累计阅览次数；没有阅览时为 0。"
  },
  {
    id: "pi",
    label: "圆周率",
...
function formulaVariables(
  ...
  return {
    ...
    view_count: Number(metrics.viewCount) || 0,
    reading_seconds: Number(metrics.readingSeconds) || 0,
    average_reading_seconds: Number(metrics.averageReadingSeconds) || 0,
...
function calculateBrightnessDetails(
  star,
  rules,
  minimumValue,
  initialValue,
  ...
  for (const rule of rules || []) {
    ...
    try {
      ...
      brightness = evaluated;
      steps.push({
        id: String(rule.id || ""),
        name: String(rule.name || rule.id || "亮度规则"),
        before,
        after: brightness,
        delta: brightness - before,
        clamped: false
      });
    } catch {
  ...
  return {
    ...
    clamped: finalBrightness !== brightness,
    steps
  };
}
...
function calculateBrightness(
  ...
  return calculateBrightnessDetails(
    star,
    rules,
    minimumValue,
    initialValue,
...
function migrateDefaultBrightnessRules(rules) {
  ...
  const matchesPreviousDefault = PREVIOUS_DEFAULT_CONFIGURATIONS.some(
    (configuration) => {
      return configuration.orders.some((order) => {
        return (
          rules.length === order.length &&
          rules.every((rule, index) => {
            return (
              rule?.id === order[index] &&
              rule.name === configuration.names.get(rule.id) &&
              rule.enabled === true &&
              rule.target === (
                rule.id.startsWith("contributor") ||
                rule.id === "rule-3f239463"
                  ? "contributor"
                  : "document"
              ) &&
              rule.formula === configuration.formulas.get(rule.id)
            );
          })
        );
      });
...
function normalizeTiers(tiers, minimumValue, maximumValue) {
  ...
  for (const [index, tier] of (tiers || []).entries()) {
    ...
    if (Number.isFinite(parsedLimit)) {
      value.max_count = Math.max(0, Math.min(10000, Math.floor(parsedLimit)));
    }
    values.push(value);
  }
...
function allocateBrightnessRanks(
  stars,
  tiers,
  minimumValue = 0,
  maximumValue = 100
  ...
  const normalized = normalizeTiers(
    tiers,
    range.minimum,
    range.maximum
  );
  const ranked = (stars || []).map((star, index) => {
    ...
    return {
      index,
      star,
      raw: Number.isFinite(raw) ? raw : range.minimum
    };
  }).sort((left, right) => {
    return (
      right.raw - left.raw ||
      String(left.star?.id || left.index).localeCompare(
        String(right.star?.id || right.index)
      )
    );
  });
  const usage = new Map();
  const allocations = new Array(ranked.length);
  for (const [rankIndex, candidate] of ranked.entries()) {
    const natural = Math.max(
      range.minimum,
      Math.min(range.maximum, candidate.raw)
    );
    const eligible = normalized.filter((tier) => {
      return natural >= tier.min_brightness;
    }).reverse();
    let selected = null;
    let slot = null;
    for (const tier of eligible) {
      const used = usage.get(tier.id) || 0;
      if (tier.max_count !== undefined && used >= tier.max_count) {
        continue;
      }
      selected = tier;
      slot = used + 1;
      usage.set(tier.id, slot);
      break;
    }
    let finalBrightness = range.minimum;
    if (selected) {
      const tierIndex = normalized.indexOf(selected);
      const nextTier = normalized[tierIndex + 1];
      const limited = selected.max_count !== undefined;
      const ceiling = nextTier
        ? nextTier.min_brightness - (limited ? slot * 0.1 : 0.1)
        : range.maximum - (limited ? (slot - 1) * 0.1 : 0);
      finalBrightness = Math.max(
        selected.min_brightness,
        Math.min(natural, ceiling)
      );
    }
    allocations[candidate.index] = {
      rawBrightness: candidate.raw,
      naturalBrightness: natural,
      finalBrightness,
      rank: rankIndex + 1,
      tier: selected,
      tierSlot: slot,
      tierLimit: selected?.max_count,
      quotaAdjusted: Math.abs(finalBrightness - natural) > 1e-9
    };
  }
  return allocations;
}
...
module.exports = {
  ...
  VARIABLES,
  allocateBrightnessRanks,
  brightnessTier,
  calculateBrightness,
  calculateBrightnessDetails,
  compileFormula,
  evaluateFormula,
  formulaVariables,
