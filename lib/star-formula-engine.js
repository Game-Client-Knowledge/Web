...
function migrateDefaultBrightnessRules(rules) {
  ...
  const matchesPreviousDefault = PREVIOUS_DEFAULT_ORDERS.some((order) => {
    return (
      ...
      rules.every((rule, index) => {
        ...
        return (
          ...
          rule.formula === PREVIOUS_DEFAULT_FORMULAS.get(rule.id)
        );
      })
    );
  });
  return matchesPreviousDefault
    ? DEFAULT_BRIGHTNESS_RULES.map((rule) => ({ ...rule }))
    : rules;
}
