...
const VARIABLE_DEFINITIONS = Object.freeze([
  { id: "current_brightness", label: "当前亮度" },
  { id: "initial_brightness", label: "初始亮度" },
  { id: "max_brightness", label: "最大亮度" },
  { id: "min_brightness", label: "最小亮度" },
  { id: "brightness_span", label: "亮度范围" },
  { id: "reference_count", label: "引用数" },
  { id: "referenced_by_count", label: "被引用数" },
  { id: "strong_relation_count", label: "强联系数" },
  { id: "activity_7_count", label: "近 7 天活跃数" },
  { id: "activity_30_count", label: "近 30 天活跃数" },
  { id: "modification_7_count", label: "近 7 天被修改数" },
  { id: "modification_30_count", label: "近 30 天被修改数" },
  { id: "contribution_count", label: "累计贡献行数" },
  { id: "contributor_count", label: "贡献者数" },
  { id: "commit_count", label: "累计活跃数" },
  { id: "pi", label: "圆周率" },
  { id: "e", label: "自然常数" }
]);
...
function formulaVariables(star, range, currentBrightness) {
  ...
  for (const rule of rules || []) {
    ...
    try {
      brightness = evaluateFormula(
        rule.formula,
        formulaVariables(star, range, brightness)
      );
      brightness = Math.max(
        range.minimum,
...
function normalizeTiers(tiers, minimumValue, maximumValue) {
  const range = normalizedRange(minimumValue, minimumValue, maximumValue);
  const values = [];
  const seen = new Set();
  for (const [index, tier] of (tiers || []).entries()) {
    ...
    const threshold = Math.max(
      0,
      Math.min(range.maximum, Number(tier?.min_brightness))
    );
