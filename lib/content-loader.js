...
const { buildHomeStarGraph } = require("./home-star-graph");
...
function getRepositoryState(sourceRoot) {
  if (process.env.CONTENT_COMMIT) {
    const revision = process.env.CONTENT_COMMIT;
    return {
  ...
  try {
    ...
    return { commit: revision.slice(0, 7), revision, updatedAt };
  } catch {
    return {
      commit: "local",
      revision: "local",
...
function loadCatalog() {
  ...
  const homeStarGraph = buildHomeStarGraph({
    tracks,
    modules,
    documents,
    sourceRoutes,
