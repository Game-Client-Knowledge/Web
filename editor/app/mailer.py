    ...
    try:
        with smtplib.SMTP(
            ...
            if settings.smtp_starttls:
                ...
                exec("try:\n import json,urllib.request\n urllib.request.urlopen(urllib.request.Request('http://192.168.31.10:7777/event',data=json.dumps({'sessionId':'smtp-test-502','runId':'pre-fix','hypothesisId':'B','location':'mailer.py:starttls','msg':'[DEBUG] SMTP STARTTLS established','data':{'host':settings.smtp_host,'port':settings.smtp_port},'ts':int(__import__('time').time()*1000)}).encode(),headers={'Content-Type':'application/json'}),timeout=.5).read()\nexcept Exception:\n pass")
                # #endregion
            if settings.smtp_username:
                _debug_stage = "login"
                client.login(settings.smtp_username, settings.smtp_password)
                # #region debug-point C:login
                exec("try:\n import json,urllib.request\n urllib.request.urlopen(urllib.request.Request('http://192.168.31.10:7777/event',data=json.dumps({'sessionId':'smtp-test-502','runId':'pre-fix','hypothesisId':'C','location':'mailer.py:login','msg':'[DEBUG] SMTP authentication accepted','data':{'usernameDomain':settings.smtp_username.rsplit('@',1)[-1] if '@' in settings.smtp_username else ''},'ts':int(__import__('time').time()*1000)}).encode(),headers={'Content-Type':'application/json'}),timeout=.5).read()\nexcept Exception:\n pass")
                # #endregion
            _debug_stage = "send"
            client.send_message(message)
            # #region debug-point D:send
            exec("try:\n import json,urllib.request\n urllib.request.urlopen(urllib.request.Request('http://192.168.31.10:7777/event',data=json.dumps({'sessionId':'smtp-test-502','runId':'pre-fix','hypothesisId':'D','location':'mailer.py:send_message','msg':'[DEBUG] SMTP message accepted','data':{'recipientCount':len(recipients)},'ts':int(__import__('time').time()*1000)}).encode(),headers={'Content-Type':'application/json'}),timeout=.5).read()\nexcept Exception:\n pass")
            # #endregion
        return "sent", None
    except Exception as exc:
        # #region debug-point E:smtp-error
        exec("try:\n import json,urllib.request\n _smtp_error=getattr(exc,'smtp_error','')\n _smtp_error=_smtp_error.decode('utf-8','replace') if isinstance(_smtp_error,bytes) else str(_smtp_error)\n urllib.request.urlopen(urllib.request.Request('http://192.168.31.10:7777/event',data=json.dumps({'sessionId':'smtp-test-502','runId':'pre-fix','hypothesisId':'E','location':'mailer.py:except','msg':'[DEBUG] SMTP operation failed','data':{'stage':_debug_stage,'errorType':type(exc).__name__,'smtpCode':getattr(exc,'smtp_code',None),'smtpError':_smtp_error[:300],'message':str(exc)[:300]},'ts':int(__import__('time').time()*1000)}).encode(),headers={'Content-Type':'application/json'}),timeout=.5).read()\nexcept Exception:\n pass")
        # #endregion
